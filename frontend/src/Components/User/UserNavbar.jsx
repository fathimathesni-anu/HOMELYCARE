...
export default function UserNavbar() {
  ...
  const handleFileChange = (e) => {
    ...
    if (!selected) return;
    setFile(selected);
    setPreview(URL.createObjectURL(selected)); // Set preview URL
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setProfilePic(res.data.profilePic); // Update image
      setFile(null);
      setPreview(null);
    } catch (err) {
            ...
            <div className="absolute top-12 right-0 w-64 bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-md shadow-lg z-50 p-4 space-y-2">
              ...
              <div className="flex items-center space-x-2">
                <div className="w-10 h-10 rounded-full overflow-hidden border">
                  <img
                    src={preview || (profilePic ? `user/uploads/${profilePic}` : 'https://ui-avatars.com/api/?name=${user.name}&background=random&size=128')}
                    alt="Preview"
